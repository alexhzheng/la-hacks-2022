      ...
      <main className="flex h-full min-h-screen justify-center items-center text-center overflow-hidden relative">
        <div className="flex flex-col gap-x-4 justify-center items-center">
          <div className="flex flex-row gap-x-2 my-4 justify-center items-center">
            Total (in USD):
            <input className="rounded border-2 border-black p-1"
              type="text"
              placeholder="0"
          ...
          <div className="flex gap-x-4">
            <input className="rounded border-2 border-black p-1"
              ...
              onChange={e => setFormData({ ...formData, user1Contact: e.target.value })} />
            <input className="rounded border-2 border-black p-1"
              type="number"
              placeholder="1"
              value={formData.user1Ratio}
              onChange={e => setFormData({ ...formData, user1Ratio: parseFloat(e.target.value) })} />
          </div>
          <div className="flex gap-x-4 my-3">
            <input className="rounded border-2 border-black p-1"
              ...
              onChange={e => setFormData({ ...formData, user2Contact: e.target.value })}
            />
            <input className="rounded border-2 border-black p-1"
              type="number"
              placeholder="1"
              value={formData.user2Ratio}
              onChange={e => setFormData({ ...formData, user2Ratio: parseFloat(e.target.value) })}
            />

          </div>
          <button
            onClick={() => {
              const amts = calcPaymentAmts(formData.total, [formData.user1Ratio, formData.user2Ratio]);
              alert(`${formData.user1Contact} owes ${amts[0]}, and ${formData.user2Contact} owes ${amts[1]}`);
            }
            }
